...
class BookHotel extends Component {
  ...
  daysBetweenDates() {
    ...
    var dateDifferenceMilliseconds = Math.abs(
      Date.parse(this.state.checkOutDate) - Date.parse(this.state.checkInDate)
    );
  ...
  minCheckOutDate() {
    ...
    var minCheckOutDate = new Date(
      millisecondsInDay + Date.parse(this.state.checkInDate)
    );
  ...
  getTotalPrice() {
    return (
      this.daysBetweenDates() * this.props.price * this.state.personsNumber
    );
  }
  openConfirm(show) {
      ...
      <Card>
        ...
        <CardSectionCentered>
          <Text style={styles.textStyle}>
            Price per night: {this.props.price} € (per person)
          </Text>
        </CardSectionCentered>
        <CardSection>
          <Text style={styles.textDateNameStyle}>Check-in</Text>
          <DatePicker
            style={{
              width: 250,
              marginLeft: 20,
            ...
            onDateChange={date => {
              this.setState({ checkInDate: date }), this.changeCheckOutDate();
            }}
          />
        </CardSection>
        <CardSection>
          <Text style={styles.textDateNameStyle}>Check-out</Text>
          <DatePicker
            style={{
              width: 250,
              marginLeft: 20,
        ...
        <CardSectionCentered>
          <Text style={styles.textStyle}>
            Total Days: {this.daysBetweenDates()}
          </Text>
        </CardSectionCentered>
        <CardSectionCentered>
          <Text style={styles.spinnerTextStyle}>Persons:</Text>
          <Spinner
            max={100}
            min={1}
            default={1}
        ...
        <CardSectionCentered>
          <Text style={styles.textTotalPriceStyle}>
        ...
        <CardSection>
          <ConfirmDialog
            ...
            message={
              "Payment of " +
              this.getTotalPrice() +
              "€ is now required to complete the booking."
            }
